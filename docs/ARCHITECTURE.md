...
- **LLM 智能生成模式（新增）**：在 `FEATURE_LLM=true` 时，用户可在档案中选择「AI 智能生成」替代传统模板渲染。LLM 直接阅读规范化后的提交/PR 事件流，按固定章节（本周完成、关键变更、风险与问题、下周计划建议）输出结构化周报。输出经 JSON Schema 校验后再渲染为 Markdown，确保事实可回溯、链接不丢失。若 LLM 调用失败或返回格式异常，自动降级为内置模板生成，保证可用性。
